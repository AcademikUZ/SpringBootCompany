...
import com.example.companyinspringboot.services.AddressService;
...
public class AddressController {
    ...
    @GetMapping("/getall")
    public ResponseEntity<Page<Address>> getAll(@RequestParam Integer page) {
        Page<Address> all = service.getAll(page);
    ...
    public ResponseEntity<?> getById(@PathVariable Long id) {
        ...
        return ResponseEntity.status(address != null ? HttpStatus.OK : HttpStatus.BAD_REQUEST).body(address);
    }

    @PutMapping("/update/{id}")
    public ResponseEntity<?> update(@Valid @PathVariable Long id, @RequestBody AddressDto addressDto) {
        ApiResult apiResult = service.update(id, addressDto);
    ...
    public ResponseEntity<?> add(@Valid @RequestBody AddressDto addressDto) {
        ApiResult apiResult = service.add(addressDto);
